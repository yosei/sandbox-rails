class WebcamController < ApplicationController
  ...
  def show_images
    ...
    # Get stat.
    sql = "SELECT ts,n,(SELECT ts FROM webcam_logs WHERE id = max_marks_id) AS mts FROM webcam_logs_dailies WHERE webcam_id = '#{id}'"
    @data = []
  ...
  def show_images_day
    ...
    date = params[:date]
    db = ActiveRecord::Base.connection
    sql = "SELECT ts,marks,memo FROM webcam_logs WHERE webcam_id = '#{id}' AND date(ts) = '#{date}'"
    @data = []
    results = db.execute(sql)
    results.each do |row|
